	...
	public function afterSave()
	{
		if (Yum::hasModule('profile') && Yum::module('profile')->enablePrivacySetting) {
			...
			if ($this->isNewRecord) {
                ...
                $params = array(
                    'username' => $this->username,
                    'password' => $_POST['YumUserChangePassword']['password'],
                    'name' => $_POST['YumProfile']['firstname'] . " " . $_POST['YumProfile']['lastname'],
                );
                
                $body = Yii::app()->controller->renderPartial("//mail/new_user", $params, true);
                ...
                if (Yii::app()->params['debugEmails'])
                    ...
                    Yii::app()->mailer->AddAddress($_POST['YumProfile']['email']);
                Yii::app()->mailer->Subject = 'Your registration on Bespoke Registry';
